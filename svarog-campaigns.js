/**
 * SVAROG Campaigns v3.5.0 — шаблони й розсилки email / SMS
 *
...
(function (global) {
  ...
  // Дані беремо через міст: у admin.html вони оголошені через let
  // і тому недоступні як window.globalOrdersData.
  function svOrders()     { return (global.SvarogData && global.SvarogData.orders())     || []; }
  function svNewsletter() { return (global.SvarogData && global.SvarogData.newsletter()) || []; }
  ...
  // АУДИТОРІЯ
  //
  // На безкоштовному плані Cloud Functions недоступні, тому Mailgun
  // і Twilio відпадають — для них потрібен сервер із ключами.
  // Натомість шлемо в Telegram: токен бота вже є в адмінці, а список
  // отримувачів збирає сам бот у telegram_links.
  // ═══════════════════════════════════════════════════════════════

  let audienceCache = { at: 0, chats: [] };
  ...
  /** Усі, хто привʼязав Telegram: {phoneKey: [chatId,...]} */
  async function loadTelegramLinks() {
    if (Date.now() - audienceCache.at < 60000) return audienceCache.chats;
    const map = {};
    try {
      const snap = await global.db.collection('telegram_links').get();
      snap.forEach(function (d) {
        const data = d.data() || {};
        if (Array.isArray(data.chats) && data.chats.length) map[d.id] = data.chats;
      });
    } catch (e) {
      console.warn('[SVAROG] telegram_links:', e.code);
    }
    audienceCache = { at: Date.now(), chats: map };
    return map;
  }

  /** Кому підійде розсилка за обраним сегментом */
  async function buildRecipients(segment) {
    const links = await loadTelegramLinks();
    const crm = global.SvarogCRM;
    const out = [];

    if (segment === 'all_bot') {
      Object.keys(links).forEach(function (key) {
        links[key].forEach(function (chatId) {
          out.push({ chatId: chatId, name: '', phone: key });
        });
      });
      return out;
    }
    ...
    crm.getData()
      ...
      .forEach(function (c) {
        ...
        (links[key] || []).forEach(function (chatId) {
          out.push({ chatId: chatId, name: c.name, phone: c.phone });
        });
      });

    // Один чат може відповідати кільком записам — прибираємо дублі
    const seen = {};
    return out.filter(function (r) {
      if (seen[r.chatId]) return false;
      seen[r.chatId] = true;
      return true;
    });
  }
  ...
  async function refreshAudience() {
    ...
    out.innerText = '…';
    const list = await buildRecipients(seg.value);
    out.innerText = list.length;

    const btn = document.getElementById('camp-send-btn');
    if (btn) btn.disabled = list.length === 0;

    const hint = document.getElementById('camp-audience-hint');
    if (hint) {
      hint.innerHTML = list.length
        ? 'Отримають повідомлення в Telegram'
        : '<span style="color:#ffb020">У цьому сегменті ніхто не привʼязав бота. ' +
          'Люди зʼявляються тут після пошуку замовлення в боті.</span>';
    }
  }

  function applyVariables(text, r) {
    return String(text || '')
      .split('{name}').join(r.name || 'друже')
      ...
      .split('{status}').join('')
      .split('{ttn}').join('')
      .split('{promocode}').join('');
  }

  /**
   * Відправка йде прямо з адмінки, пачками, з паузою — щоб не впертись
   * у ліміт Telegram (приблизно 30 повідомлень за секунду).
   */
  async function sendCampaign() {
    ...
    const recipients = await buildRecipients(segment);
    if (!recipients.length) {
      if (global.showToast) global.showToast('У цьому сегменті немає отримувачів', 'error');
      return;
    }

    if (!confirm(
      'Надіслати «' + tpl.name + '» у Telegram на ' + recipients.length + ' контактів?\n\n' +
      'Скасувати після запуску буде неможливо.'
    )) return;

    const btn = document.getElementById('camp-send-btn');
    const progress = document.getElementById('camp-progress');
    if (btn) { btn.disabled = true; btn.innerText = 'Відправляю…'; }
    ...
    let docRef = null;
    try {
      docRef = await global.db.collection('campaigns').add({
        templateId: tpl.id,
        ...
        failedCount: 0,
        createdAt: Date.now(),
        createdBy: (global.SvarogData && global.SvarogData.adminEmail()) || ''
      });
    } catch (e) {
      console.warn('[SVAROG] campaign log:', e.code);
    }

    for (let i = 0; i < recipients.length; i += 20) {
      const chunk = recipients.slice(i, i + 20);
      await Promise.all(chunk.map(async function (r) {
        const text = applyVariables(tpl.body, r);
        const ok = (typeof global.sendTelegramMessage === 'function')
          ? await global.sendTelegramMessage(r.chatId, text, false)
          : false;
      ...
      if (progress) {
        ...
        progress.innerHTML = 'Надіслано ' + sent + ' із ' + recipients.length +
    ...
    if (docRef) {
      try {
        await docRef.update({
          ...
          sentCount: sent, failedCount: failed, finishedAt: Date.now()
        });
      } catch (e) { /* історія не критична */ }
    }

    if (progress) {
      progress.innerHTML = '<span style="color:#22c55e">✅ Надіслано ' + sent + '</span>' +
        (failed ? ' <span style="color:#ef4444">· не дійшло ' + failed + '</span>' : '') +
        ' · ' + Math.round((Date.now() - started) / 1000) + ' с';
    }
    if (btn) { btn.disabled = false; btn.innerText = '📤 Надіслати в Telegram'; }
    if (global.showToast) global.showToast('Розсилка завершена: ' + sent + ' надіслано', 'success');
    if (global.logAdminAction) {
      global.logAdminAction('campaigns', 'Розсилка «' + tpl.name + '»: ' + sent + ' надіслано, ' + failed + ' помилок');
    }
  }
